	...
	if (isset($parameters['title'])) {
		if($parameters['title'] == 'TYPO3 Edit Document'){
			...
			if (is_array($_editArray)) {
				...
				if ($table == 'tx_phpdisplay_displays') {
					
					//... before injected some javascript
					$pObj->loadJavascriptLib('js/common.js');
					$pObj->loadJavascriptLib(t3lib_extMgm::extRelPath('phpdisplay') . 'resources/javascript/formatJson.js');
					$pObj->loadJavascriptLib(t3lib_extMgm::extRelPath('phpdisplay') . 'resources/javascript/phpdisplay.js');
					$pObj->inDocStylesArray['phpdisplay'] = file_get_contents(t3lib_extMgm::extPath('phpdisplay').'resources/css/phpdisplay.css');
				}
			}
		}
	}
