...
/**
 * This class answers to AJAX calls from the 'phpdisplay' extension
 *
 * @author	Fabien Udriot <fabien.udriot@ecodev.ch>
 * @package	TYPO3
 * @subpackage	tx_phpdisplay
 */
class tx_phpdisplay_ajax {
	...
	public function saveConfiguration($params, $ajaxObj) {
		...
		$record = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('uid', 'tx_phpdisplay_displays', 'uid = '. $uid);
		
		$result = 0;
		
		if (!empty($record)) {
			$updateArray['mappings'] = $mappings;
			$msg = $GLOBALS['TYPO3_DB']->exec_UPDATEquery('tx_phpdisplay_displays', 'uid = '. $uid, $updateArray);
			
			if ($msg == 1) {
				$result = 1;
			}
		}
		$ajaxObj->addContent('phpdisplay', $result);
	}
	...
	public function saveTemplate($params, $ajaxObj) {
		...
		$result = 0;
		$tceforms = t3lib_div::makeInstance('tx_phpdisplay_tceforms');
			...
			if ($msg == 1) {
                ...
                if (preg_match('/^FILE:/isU', $template)) {
                	...
                	if (is_file($filePath)) {
                		...
                		global $LANG;
	                	$template = $LANG->sL('LLL:EXT:phpdisplay/locallang_db.xml:tx_phpdisplay_displays.fileNotFound') . ' ' . $template;
                	}
                }
