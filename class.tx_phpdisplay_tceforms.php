 ...
 * $Id$
 */
class tx_phpdisplay_tceforms {
	protected $extKey = 'phpdisplay';
	...
	public function mappingField($PA, $fobj) {
		...
		try {
			...
			try {
				...
				#$templateFile = t3lib_div::getFileAbsFileName('uploads/tx_phpdisplay/' . $row['template']);
				...
				if($row['template'] == ''){
					$templateContent = $this->getLL('tx_phpdisplay_displays.noTemplateFoundError');
				}
					...
					$marker['###IMPORTED###'] = '(' . $this->getLL('tx_phpdisplay_displays.imported') . ')';
					if (is_file($filePath)) {
					 	$templateContent = file_get_contents($filePath);
						$templateContent = str_replace('	', '  ', $templateContent);
					}
					else {
					 	$templateContent = $this->getLL('tx_phpdisplay_displays.fileNotFound') . ' ' . $row['template'];
					}
				}
				...
				$marker['###INFOMODULE_PATH###'] = t3lib_extMgm::extRelPath('phpdisplay').'resources/images/';
				$marker['###UID###'] = $row['uid'];
				$marker['###TEXT###'] = $this->getLL('tx_phpdisplay_displays.text');
				$marker['###RICHTEXT###'] = $this->getLL('tx_phpdisplay_displays.richtext');
				$marker['###IMAGE###'] = $this->getLL('tx_phpdisplay_displays.image');
				$marker['###IMAGE_RESOURCE###'] = $this->getLL('tx_phpdisplay_displays.image_resource');
				$marker['###LINK_TO_DETAIL###'] = $this->getLL('tx_phpdisplay_displays.link_to_detail');
				$marker['###LINK_TO_PAGE###'] = $this->getLL('tx_phpdisplay_displays.link_to_page');
				$marker['###LINK_TO_FILE###'] = $this->getLL('tx_phpdisplay_displays.link_to_file');
				$marker['###USER###'] = $this->getLL('tx_phpdisplay_displays.user');
				$marker['###EMAIL###'] = $this->getLL('tx_phpdisplay_displays.email');
				$marker['###SHOW_JSON###'] = $this->getLL('tx_phpdisplay_displays.showJson');
				$marker['###EDIT_JSON###'] = $this->getLL('tx_phpdisplay_displays.editJson');
				$marker['###EDIT_HTML###'] = $this->getLL('tx_phpdisplay_displays.editHtml');
				$marker['###MAPPING###'] = $this->getLL('tx_phpdisplay_displays.mapping');
				$marker['###TYPES###'] = $this->getLL('tx_phpdisplay_displays.types');
				$marker['###FIELDS###'] = $this->getLL('tx_phpdisplay_displays.fields');
				$marker['###CONFIGURATION###'] = $this->getLL('tx_phpdisplay_displays.configuration');
				$marker['###SAVE_FIELD_CONFIGURATION###'] = $this->getLL('tx_phpdisplay_displays.saveFieldConfiguration');

				# Parse the template and render it.
				$backendTemplatefile = t3lib_div::getFileAbsFileName('EXT:phpdisplay/resources/templates/phpdisplay.html');
	...
	public function transformTemplateContent($templateContent) {
		...
		$replacement[] = '<span class="phpdisplay_if">$1</span>';
		
		$pattern[] = "/(&lt;!-- *EMPTY *--&gt;|&lt;!-- *ENDEMPTY *--&gt;)/isU";
		$replacement[] = '<span class="phpdisplay_empty">$1</span>';

		$pattern[] = "/(#{3}.+#{3})/isU";
		$replacement[] = '<span class="phpdisplay_label">$1</span>';
		...
		$replacement[] = '<span class="phpdisplay_function">$1</span>';

		# Wrap LOOP markers with a different background
		$pattern[] = "/(&lt;!-- *LOOP *\(.+--&gt;)/isU";
		$replacement[] = '<span class="phpdisplay_loop">$1</span>';
		...
		$path = t3lib_extMgm::extRelPath('phpdisplay').'resources/images/';
	...
	private function getLL($key){
		$langReference = 'LLL:EXT:phpdisplay/locallang_db.xml:';
	...
	protected function getRelatedProvider($row) {
		...
		$mmTables = $this->getMMTablesList();
			// In each table, try to find relations to the current phpdisplay component
		foreach ($mmTables as $aTable) {
			$relations = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('uid_local, local_table, local_field', $aTable, "uid_foreign = '" . $row['uid'] . "' AND tablenames = 'tx_phpdisplay_displays'");
		...
		if ($numRelations == 0) {
			...
			// NOTE:	a phpdisplay component may be related to several providers
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/phpdisplay/class.tx_phpdisplay_tceforms.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/phpdisplay/class.tx_phpdisplay_tceforms.php']);
}
